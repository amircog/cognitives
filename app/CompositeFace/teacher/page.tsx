...
import {
  ...
  ResponsiveContainer, ErrorBar, ScatterChart, Scatter, ZAxis, Cell,
} from 'recharts';
...
// ── Reveal wrapper ────────────────────────────────────────────────────────────
// Each chart card has a "Reveal" button. Until clicked, only axes/labels show.
...
function ChartCard({ title, subtitle, revealed, onReveal, children }: ChartCardProps) {
  return (
...
function AccChart({ data, revealed }: { data: BarPoint[]; revealed: boolean }) {
  return (
    <ResponsiveContainer width="100%" height={230}>
      <BarChart data={data} margin={{ left: 10, bottom: 5 }} barCategoryGap="30%">
...
function RTChart({ data, revealed }: { data: BarPoint[]; revealed: boolean }) {
  return (
    <ResponsiveContainer width="100%" height={230}>
      <BarChart data={data} margin={{ left: 10, bottom: 5 }} barCategoryGap="30%">
        <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
        <XAxis dataKey="name" tick={TICK} />
        <YAxis domain={[0, 1500]} tick={TICK}
...
function ScatterPlot({ data, revealed }: { data: ScatterPoint[]; revealed: boolean }) {
  return (
    <ResponsiveContainer width="100%" height={260}>
      <ScatterChart margin={{ left: 10, bottom: 20, right: 20, top: 10 }}>
        <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
        <XAxis dataKey="x" type="number" name="Aligned Acc" domain={[0, 100]}
          label={{ value: 'Aligned Accuracy (%)', position: 'insideBottom', offset: -12, style: LBL }}
          tick={TICK} />
        <YAxis dataKey="y" type="number" name="Misaligned Acc" domain={[0, 100]}
          label={{ value: 'Misaligned Accuracy (%)', angle: -90, position: 'insideLeft', style: LBL }}
          tick={TICK} />
        <ZAxis range={[60, 60]} />
        <Tooltip content={<ScatterTooltip />} />
        {/* Diagonal y=x — always visible as reference */}
        <Scatter
...
export default function TeacherPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setData(rows.length > 0 ? computeTeacherData(rows) : null);
    } catch (e) {
  ...
  const handleDownloadCSV = async () => {
    try {
      ...
      a.href = url; a.download = 'composite-face-data.csv'; a.click();
      ...
      <div className="container mx-auto px-4 py-8 max-w-5xl">
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-6">
            ...
            {/* Chart 1 — Accuracy: aligned vs. misaligned */}
            <ChartCard
              title="Accuracy: Aligned vs. Misaligned"
              subtitle="Misaligned = average of small + large. Error bars = SEM."
              revealed={!!revealed[1]}
              onReveal={() => reveal(1)}
            >
              <AccChart data={data.chart1} revealed={!!revealed[1]} />
            </ChartCard>

            {/* Chart 2 — RT: aligned vs. misaligned */}
            <ChartCard
              title="Reaction Time: Aligned vs. Misaligned"
              subtitle="Correct trials only. Error bars = SEM."
              revealed={!!revealed[2]}
              onReveal={() => reveal(2)}
            >
              <RTChart data={data.chart2} revealed={!!revealed[2]} />
            </ChartCard>

            {/* Charts 3 & 4 — small vs. large */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <ChartCard
                title="Accuracy: Small vs. Large Misalignment"
                revealed={!!revealed[3]}
                onReveal={() => reveal(3)}
              >
                <AccChart data={data.chart3} revealed={!!revealed[3]} />
              </ChartCard>
              <ChartCard
                title="RT: Small vs. Large Misalignment"
                subtitle="Correct trials only."
                revealed={!!revealed[4]}
                onReveal={() => reveal(4)}
              >
                <RTChart data={data.chart4} revealed={!!revealed[4]} />
              </ChartCard>
            </div>

            {/* Chart 5 — Scatter */}
            <ChartCard
              title="Individual Participants: Aligned vs. Misaligned Accuracy"
            ...
            >
              {data.chart5.length === 0 ? (
                ...
                <ScatterPlot data={data.chart5} revealed={!!revealed[5]} />
              )}
            </ChartCard>
